    ...
    for cam_name in all_tracklet:
        if cam_name == "c041":
            ...
            for tracklet in all_tracklet[cam_name]:
                ...
                start_frame_id = tracklet["start_frame_id"]
                end_frame_id = tracklet["end_frame_id"]
                # Kết thúc trong vùng xuất phát
                if end_region_id in start_region_ids:   
                    if end_frame_id != 2000:
                        if cal_boxes_avg_area(tracklet["box_list"]) > 700:
                            tracklets.append(tracklet)
                            continue
                # Kết thúc vùng giữa
                if end_region_id in middle_regions:   
                    if end_frame_id != 2000:
                        if cal_boxes_avg_area(tracklet["box_list"]) > 700:
                            tracklets.append(tracklet)
                            continue
                # Không bắt đầu ở vùng xuất phát
                if(end_region_id in end_region_ids) and (start_region_id not in start_region_ids):
                    if start_frame_id != 0 and end_frame_id != 2000:
                        if cal_boxes_avg_area(tracklet["box_list"]) > 700:
                            tracklets.append(tracklet)
                            continue
                # Bbox đầu tiên xuất hiện ở vung
