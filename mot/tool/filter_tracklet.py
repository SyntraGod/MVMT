...
cam_dis = {"c041-c042":351,"c042-c043":160,"c043-c044":304}
# cam_avg_erae = {"c041":700,"c042":700,"c043":700,"c044":700,"c045":700,"c046":700}
cam_avg_erae = {"c041":700,"c042":700,"c043":700,"c044":700}

# Điều chỉnh thông tin của vùng
...
# Kiểm tra tính hợp lệ các vùng mà tracklet đi qua
    ...
    for cam_name in all_tracklet:
        if cam_name == "c041":
            for tracklet in all_tracklet[cam_name]:
                ...
                track_id = tracklet["track_id"]
                end_frame_id = tracklet["end_frame_id"]
                # VÙng đi qua rỗng --> báo lỗi
                if len(zone_list) == 0:
                    print("cam:{},go_through_region is error track id:{}".format(cam_name,track_id))
                    raise
                # vùng mà tracklet kết thúc khác với vùng kết thúc trong tracklet
                ...
                if zone_list[0] != start_region_id:
                    ...
                    raise
                # Track kết thúc giữa chừng và biến mất
                if  zone_list[-1] not in end_region_ids and len([i for i in zone_list if i in end_region_ids]) > 0:
                    if end_frame_id != 2000:
...
# Lọc bất thường liên quan đến vị trí, hướng di chuyển
def filter_tracklet_distance(all_tracklet,new_all_mask):
    for cam_name in all_tracklet:
        if cam_name == "c041":
            '''
            "sv": chứa các track_id của các tracklet có số lượng giá trị feature nhỏ hơn hoặc bằng 5.
            "suo": chứa các track_id của các tracklet mà có vùng tương tác với bên ngoài thông qua tính toán IOU (Intersection over Union).
            "suf": chứa các track_id của các tracklet có sự thay đổi lớn về khung thời gian (các tracklet kéo dài hơn 1000 khung hình).
            "sel": chứa các track_id của các tracklet có diện tích trung bình nhỏ hơn một ngưỡng xác định (diện tích nhỏ và có ít hơn 50 hộp bao).
            '''
