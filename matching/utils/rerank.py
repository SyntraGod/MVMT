...
# Tái xếp hạng các đặc trưng
# k1 số hàng xóm trong giai đoạn khởi tạo, k2 là số hàng xóm trong giai đoạn tái xếp hạng
def ReRank2(probFea, galFea, k1=20, k2=6, lambda_value=0.3):
    ...
    # Tính toán thứ hạng ban đầu, tìm ra các hàng xóm gần nhất
    initial_rank = batch_torch_topk(feat, feat, min(k1 + 1, len(probFea) + 1), N=6000)
    # del feat
    del probFea
    del galFea
    torch.cuda.empty_cache()  # empty GPU memory
